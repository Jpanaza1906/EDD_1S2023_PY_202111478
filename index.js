...
fetch("ProyectoF1/test.json")
    ...
    .then(data => {
        ...
        for (let i = 0; i < data.alumnos.length; i++){
            var student = new Estudiante(alumno[i].nombre,parseInt(alumno[i].carnet),alumno[i].password);
            arbolBinarioAVL.insertaEstudiante(student);
        }
    })
...
function login(){
    ...
    if(user === "admin" && password === "admin"){
        ...
        window.location.replace("EDD_Proyecto1_Fase2/mainadmin.html");
    }
